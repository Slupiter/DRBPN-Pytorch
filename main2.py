...
parser.add_argument('--start_iter', type=int, default=1300, help='Starting Epoch')
...
parser.add_argument('--pretrained_sr', default='DIV2K_train_HRD-DRBPN2_recursion3_epoch_1299.pth', help='sr pretrained base model')
...
def checkpoint(epoch):
    ...
    save_dir=os.path.join(opt.save_folder,opt.model_type,str(opt.upscale_factor))
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    model_out_path = save_dir + '/' + opt.train_dataset+opt.model_type+str(opt.upscale_factor)+'_recursion'+str(opt.recursion) +"_epoch_{}.pth".format(epoch)
...
if opt.pretrained:
    model_name = os.path.join(opt.save_folder, opt.model_type, str(opt.upscale_factor), opt.pretrained_sr)
    if os.path.exists(model_name):
        #model= torch.load(model_name, map_location=lambda storage, loc: storage)
        model.load_state_dict(torch.load(model_name, map_location=lambda storage, loc: storage))
        print('Pre-trained SR model is loaded.')
        #p=[int(s) for s in re.findall(r'\b\d+\b', opt.pretrained_sr)]
if cuda:
