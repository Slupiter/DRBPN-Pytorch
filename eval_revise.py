...
test_set = get_eval_set_original(os.path.join(opt.input_dir,opt.test_dataset), opt.upscale_factor)
...
def eval():
    ...
    for batch in testing_data_loader:
        ...
        t0 = time.time()
        # 原文是因为caffe处理能力有限，选择将图片分成四部分处理
        if opt.chop_forward:
            with torch.no_grad():
                prediction = chop_forward(input, model, opt.upscale_factor)
        else:
        # EDSR中提出测试时以自相似方法进行可以提高PSNR结果 
        ...
        psnr = skimage.measure.compare_psnr(prediction, target, data_range=255)
        avg_psnr += psnr
        ssim = skimage.measure.compare_ssim(prediction, target, multichannel=True)
        avg_ssim += ssim
        t1 = time.time()
        print("===> Processing: %s || Timer: %.4f sec. || psnr: %.4f db || ssim: %.4f" % (name[0], (t1 - t0), psnr, ssim))
