...
class DatasetFromFolder(data.Dataset):
  ...
  #      ########
def get_patch_eval(img_in, img_tar, img_bic, patch_size, scale, ix=-1, iy=-1):
    (ih, iw) = img_in.size
    (th, tw) = (scale * ih, scale * iw)
...
# 将随机值修改成固定值方便后期各种模型比较相同位置图片
    ...
    #将源代码中直接评价退化图片的部分修改成从原图片下采样后再进行评价并计算PSNR,SSIM
class DatasetFromFolderEvalOriginal(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        input, target, bicubic, position = get_patch_eval(input,target,bicubic,self.patch_size, self.upscale_factor)
        # 打印位置坐标 
        print(position)
